"""
AI Foundry Branding & Content Consistency Agent implementation.
Adapted from the ADK agent pattern to work with Azure AI Foundry for brand-safe
content ideation, design guidance, and review workflows.
...
class FoundryBrandingAgent:
    """
    AI Foundry Branding & Content Consistency Agent for creative direction and review.
    This class adapts the ADK agent pattern for Azure AI Foundry with a focus on
    grounding every response in the company's branding guidelines, ensuring
    visuals, tone, and copy remain aligned across campaigns and content types.
    ...
    # Class-level shared resources for branding document search (created once)
    ...
    async def _setup_file_search(self, files_directory: str = "documents") -> Optional[FileSearchTool]:
        ...
        async with FoundryBrandingAgent._file_search_setup_lock:
            # If we already have a shared file search tool, return it
            if FoundryBrandingAgent._shared_file_search_tool is not None:
                logger.info("Reusing existing shared file search tool")
                return FoundryBrandingAgent._shared_file_search_tool
            ...
            try:
                ...
                for file_path in file_paths:
                    try:
                        ...
                        FoundryBrandingAgent._shared_uploaded_files.append(file.id)
                ...
                FoundryBrandingAgent._shared_vector_store = project_client.agents.vector_stores.create_and_poll(
                    file_ids=file_ids, 
                    name="branding_vectorstore"
                )
                logger.info(f"Created shared vector store: {FoundryBrandingAgent._shared_vector_store.id}")
                
                # Create file search tool ONCE
                file_search = FileSearchTool(vector_store_ids=[FoundryBrandingAgent._shared_vector_store.id])
                ...
                FoundryBrandingAgent._shared_file_search_tool = file_search
    ...
    async def create_agent(self) -> Agent:
        """Create the AI Foundry agent with web search and branding guideline search capabilities."""
        if self.agent:
            logger.info("Branding agent already exists, returning existing instance")
            return self.agent
        ...
        logger.info(f"Created AI Foundry Branding agent: {self.agent.id}")
        return self.agent
    
    def _get_agent_instructions(self) -> str:
        """Get the agent instructions for brand-aligned content strategy and review."""
        return f"""
You are an intelligent branding and content consistency specialist powered by Azure AI Foundry.

Your mission is to ideate, review, and refine creative outputs so that every asset reflects the approved Company Branding Guide located in the `documents/` directory (notably `company_branding.md`).

## Core Responsibilities

1. **Content Ideation & Briefing** – Produce campaign concepts, scripts, prompts, captions, and UI copy that embody the brand tone (professional, confident, innovative) while highlighting key value propositions.
2. **Visual Direction** – Translate the branding palette, lighting, and composition rules into clear art direction for designers or generative tools. When recommending imagery, reference brand colors, lighting style, typography, and composition from the guide.
3. **Brand Compliance Review** – Audit provided copy or concepts for alignment with brand voice, terminology, and prohibited elements. Suggest revisions to bring content back on-brand.
4. **Prompt Engineering** – Craft image or video generation prompts that embed the brand's colors, mood, and stylistic cues. When relevant, include negative prompts to avoid off-brand outcomes.
5. **Cross-Channel Adaptation** – Tailor messaging and visuals for specific channels (social, web, presentations, ads) while preserving core brand identity.

## Operating Guidelines

- Always consult `company_branding.md` via file search before finalizing recommendations. Cite the relevant sections to justify key decisions (e.g., color palette, tone, composition).
...
**Tone & Voice Notes**: <voice guidance mapped to brand attributes>

**Visual Direction**:
- Palette & Lighting: <colors, lighting, mood> (cite guide)
- Composition & Style: <layout, depth, iconography> (cite guide)
- Human/Tech Elements: <representation guidance> (cite guide)

**Creative Prompts / Copy Drafts**:
- <prompt or copy sample>

**Review & Next Steps**:
- <risks, follow-ups, approvals needed>
...
async def create_foundry_branding_agent() -> FoundryBrandingAgent:
    """Factory function to create and initialize a Foundry Branding agent."""
    agent = FoundryBrandingAgent()
...
async def demo_agent_interaction():
    """Demo function showing how to use the Foundry Branding agent for content guidance."""
    agent = await create_foundry_branding_agent()
    ...
    try:
        ...
        # Example interaction
        message = "We need LinkedIn carousel copy and imagery direction for our AI automation product launch."
        print(f"\nUser: {message}")
        ...
        # await FoundryBrandingAgent.cleanup_shared_resources()
