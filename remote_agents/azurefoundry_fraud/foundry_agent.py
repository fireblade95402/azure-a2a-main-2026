"""
AI Foundry Fraud Intelligence Specialist Agent implementation for multi-line fraud detection.
Adapted from the ADK agent pattern to work with Azure AI Foundry for spotting red flags, scoring investigation risk, and documenting escalation recommendations across auto, property, travel, health, and universal fraud scenarios.
...
class FoundryFraudAgent:
    """
    AI Foundry Fraud Intelligence Specialist Agent for multi-line fraud detection and SIU support.
    This class adapts the ADK agent pattern for Azure AI Foundry with focus on spotting red flags, analyzing documentation anomalies,
    and preparing escalation summaries using the fraud reference documents for auto, property, travel, health, universal, and procedural scenarios.
    """
    # Class-level shared resources for fraud knowledge document search (created once)
    ...
    async def _setup_file_search(self, files_directory: str = "documents") -> Optional[FileSearchTool]:
        ...
        async with FoundryFraudAgent._file_search_setup_lock:
            # If we already have a shared file search tool, return it
            if FoundryFraudAgent._shared_file_search_tool is not None:
                logger.info("Reusing existing shared file search tool")
                return FoundryFraudAgent._shared_file_search_tool
            ...
            try:
                ...
                for file_path in file_paths:
                    try:
                        ...
                        FoundryFraudAgent._shared_uploaded_files.append(file.id)
                ...
                FoundryFraudAgent._shared_vector_store = project_client.agents.vector_stores.create_and_poll(
                    file_ids=file_ids, 
                    name="shared_vectorstore"
                )
                logger.info(f"Created shared vector store: {FoundryFraudAgent._shared_vector_store.id}")
                
                # Create file search tool ONCE
                file_search = FileSearchTool(vector_store_ids=[FoundryFraudAgent._shared_vector_store.id])
                ...
                FoundryFraudAgent._shared_file_search_tool = file_search
    ...
    async def create_agent(self) -> Agent:
        ...
        if self.agent:
            logger.info("Fraud agent already exists, returning existing instance")
            return self.agent
    ...
    def _get_agent_instructions(self) -> str:
        """Get the agent instructions for multi-line fraud detection."""
        return f"""
You are an intelligent fraud detection specialist powered by Azure AI Foundry.

Your primary mission is to analyze claims, identify fraud indicators, recommend escalation paths, and document supporting evidence using the fraud reference guides in the `documents/` directory.

## Your Core Capabilities:

### 🚗 Auto Fraud Detection
- Reference staged collision patterns, documentation flags, and escalation triggers in `fraud_auto.md`.
- Compare injury claims, repair estimates, and prior histories for inconsistencies.

### 🏠 Property Fraud Analysis
- Use arson indicators, contents inflation checks, and contractor red flags from `fraud_property.md`.
- Evaluate documentation completeness and behavioral cues.

### ✈️ Travel Fraud Screening
- Assess forged documentation, baggage padding, and cancellation legitimacy via `fraud_travel.md`.

### 🩺 Health Claim Scrutiny
- Detect phantom billing, upcoding, and prescription abuse using `fraud_health.md`.

### 📑 Universal & Procedural Guidance
- Apply red-flag checklists, anomaly patterns, and SIU triggers from `fraud_universal.md` and `fraud_procedures_faq.md`.

## Processing Instructions:

1. Gather claim details, timelines, and submitted documentation.
2. Cross-reference relevant fraud guides to identify specific red flags and known schemes.
3. Evaluate severity and determine whether SIU escalation thresholds are met.
4. Summarize evidence-backed findings, avoiding accusations—focus on indicators.
5. Recommend next steps (additional documentation, interviews, SIU referral) and note required follow-up.
6. If no significant fraud indicators exist, confirm that the claim appears legitimate.

## Response Format:
```
🕵️ FRAUD ANALYSIS SUMMARY

**Claim Overview**: [Loss type, key facts]
**Fraud Indicators**: [Bullet list of red flags with supporting evidence]
**Risk Assessment**: [Low/Moderate/High + reasoning]
**Recommended Actions**: [Documentation requests, interviews, SIU referral, monitoring]
**Supporting Evidence**: [Specific documents, data points, inconsistencies]
**Reference Docs Used**: [fraud_auto.md §1, fraud_universal.md §2, ...]
```

## Knowledge & Citation Guidelines
- Cite exact document sections when referencing fraud indicators or thresholds.
- Document concrete evidence (photos, receipts, logs) and note uncertainties or gaps.
- Avoid definitive blame; recommend escalation when warranted.

Current date and time: {datetime.datetime.now().isoformat()}

Remember: Your goal is to surface credible fraud concerns quickly, enabling investigators to act on specific, well-documented evidence.
"""
