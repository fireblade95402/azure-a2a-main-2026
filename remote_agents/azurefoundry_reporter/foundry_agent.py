"""
Azure AI Foundry Reporter Agent
================================

A specialized report creation agent that generates professional, structured reports
based on user requirements. This agent uses predefined templates to ensure consistency
...
- Custom Reports with flexible sections
...
class FoundryReporterAgent:
    """
    Azure AI Foundry Reporter Agent
    
    A specialized agent for creating professional, structured reports.
    It can generate various types of reports using predefined templates,
    ensuring consistency and high-quality output.
    ...
    async def _setup_file_search(self, files_directory: str = "documents") -> Optional[FileSearchTool]:
        ...
        async with FoundryReporterAgent._file_search_setup_lock:
            # If we already have a shared file search tool, return it
            if FoundryReporterAgent._shared_file_search_tool is not None:
            ...
            try:
                ...
                for file_path in file_paths:
                    try:
                        ...
                        FoundryReporterAgent._shared_uploaded_files.append(file.id)
                ...
                FoundryReporterAgent._shared_vector_store = project_client.agents.vector_stores.create_and_poll(
                    file_ids=file_ids, 
                    name="reporter_agent_vectorstore"
                )
                logger.info(f"Created shared vector store: {FoundryReporterAgent._shared_vector_store.id}")
                
                # Create file search tool ONCE
                file_search = FileSearchTool(vector_store_ids=[FoundryReporterAgent._shared_vector_store.id])
                ...
                FoundryReporterAgent._shared_file_search_tool = file_search
    ...
    def _get_agent_instructions(self) -> str:
        ...
        This agent creates professional, structured reports using predefined templates.
        """
        return f"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  PROFESSIONAL REPORT CREATOR AGENT                                            ║
╚══════════════════════════════════════════════════════════════════════════════╝

You are a Professional Report Creator Agent powered by Azure AI Foundry.
Your role is to create well-structured, professional reports based on user requirements.

## Core Responsibilities

1. **Report Generation** – Create comprehensive, professional reports on any topic
2. **Template Application** – Use structured templates to ensure consistency
3. **Research & Analysis** – Gather information using web search when needed
4. **Document Integration** – Reference uploaded documents when available

## Report Types You Can Create

### 1. Executive Summary Report
Use when: User needs a high-level overview for leadership/stakeholders
```
...
Use when: User needs detailed technical evaluation
```
...
Use when: User needs project progress update
```
...
|-------|--------|-----------------|
| [Issue 1] | [High/Medium/Low] | [Plan] |

## 📊 KEY METRICS
- Budget: [X]% utilized
- Timeline: [X] days [ahead/behind]
- Resources: [Status]

## 🔮 OUTLOOK
[Next period forecast and focus areas]

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

### 4. Custom Report
Use when: User has specific format requirements
- Ask user for section requirements
- Adapt template structure to their needs
...
async def create_foundry_reporter_agent() -> FoundryReporterAgent:
    """Factory function to create and initialize a Foundry Reporter agent."""
    agent = FoundryReporterAgent()
...
async def demo_agent_interaction():
    """Demo function showing how to use the Foundry Reporter agent."""
    agent = await create_foundry_reporter_agent()
    
    try:
        # Create a conversation thread
        thread = await agent.create_thread()
        
        # Example interaction for the Reporter agent
        message = "Create an executive summary report about the current state of AI in enterprise software development."
        print(f"\nUser: {message}")
        ...
        # await FoundryReporterAgent.cleanup_shared_resources()
