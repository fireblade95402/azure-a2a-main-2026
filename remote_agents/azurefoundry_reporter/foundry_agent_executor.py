"""
Azure AI Foundry Reporter Agent - Executor
==========================================

This executor handles A2A protocol integration for the Reporter agent.
"""
import asyncio
import logging
import base64
...
from foundry_agent import FoundryReporterAgent
...
class FoundryReporterAgentExecutor(AgentExecutor):
    """
    An AgentExecutor for the Foundry Reporter agent.
    ...
    _shared_foundry_agent: Optional[FoundryReporterAgent] = None
    ...
    @classmethod
    async def get_shared_agent(cls) -> Optional[FoundryReporterAgent]:
    ...
    async def initialize_at_startup(cls) -> None:
        ...
        async with cls._agent_lock:
            if not cls._shared_foundry_agent:
                logger.info("🚀 Initializing Foundry Reporter agent at startup...")
                try:
                    cls._shared_foundry_agent = FoundryReporterAgent()
                    await cls._shared_foundry_agent.create_agent()
                    cls._startup_complete = True
                    logger.info("✅ Foundry Reporter agent startup initialization completed successfully")
                except Exception as e:
                    logger.error(f"❌ Failed to initialize Reporter agent at startup: {e}")
    ...
    async def _get_or_create_agent(self) -> FoundryReporterAgent:
        """Get the shared Foundry Reporter agent (with fallback to lazy creation)."""
        async with FoundryReporterAgentExecutor._agent_lock:
            if not FoundryReporterAgentExecutor._shared_foundry_agent:
                if FoundryReporterAgentExecutor._startup_complete:
                    # Startup was supposed to happen but failed
                    raise RuntimeError("Reporter agent startup initialization failed - agent not available")

                # Fallback to lazy creation if startup wasn't called
                logger.warning("⚠️ Reporter agent not initialized at startup, falling back to lazy creation...")
                FoundryReporterAgentExecutor._shared_foundry_agent = FoundryReporterAgent()
                await FoundryReporterAgentExecutor._shared_foundry_agent.create_agent()
                logger.info("Fallback Reporter agent creation completed")
            return FoundryReporterAgentExecutor._shared_foundry_agent

    async def _get_or_create_thread(
        self,
        context_id: str,
        agent: Optional[FoundryReporterAgent] = None
    ) -> str:
        if agent is None:
    ...
    async def _process_request(
        ...
        if received_files:
            ...
            print("[Reporter Executor] Received file references:")
    ...
    async def execute(
        ...
        async with FoundryReporterAgentExecutor._request_semaphore:
            # Check API call rate limiting
            current_time = time.time()
            
            # Reset the window if it's been more than a minute
            if current_time - FoundryReporterAgentExecutor._api_call_window_start > 60:
                FoundryReporterAgentExecutor._api_call_count = 0
                FoundryReporterAgentExecutor._api_call_window_start = current_time
            
            # Check if we're approaching the API limit
            if FoundryReporterAgentExecutor._api_call_count >= FoundryReporterAgentExecutor._max_api_calls_per_minute:
                wait_time = 60 - (current_time - FoundryReporterAgentExecutor._api_call_window_start)
                if wait_time > 0:
                    logger.warning(f"API rate limit protection: waiting {wait_time:.1f}s to reset window")
                    await asyncio.sleep(wait_time)
                    # Reset counters
                    FoundryReporterAgentExecutor._api_call_count = 0
                    FoundryReporterAgentExecutor._api_call_window_start = time.time()
            
            # Enforce minimum interval between requests - THIS IS THE KEY FIX
            time_since_last = current_time - FoundryReporterAgentExecutor._last_request_time
            if time_since_last < FoundryReporterAgentExecutor._min_request_interval:
                sleep_time = FoundryReporterAgentExecutor._min_request_interval - time_since_last
            ...
            FoundryReporterAgentExecutor._last_request_time = time.time()
...
def create_foundry_reporter_executor(card: AgentCard) -> FoundryReporterAgentExecutor:
    return FoundryReporterAgentExecutor(card)


async def initialize_foundry_reporter_agents_at_startup():
    """
    Convenience function to initialize shared Reporter agent resources at application startup.
    ...
    import asyncio
    from foundry_agent_executor import initialize_foundry_reporter_agents_at_startup

    async def startup():
        print("🚀 Starting Reporter agent application...")
        await initialize_foundry_reporter_agents_at_startup()
        print("✅ Reporter Agent initialization complete, ready to handle requests")

    # Run at startup
    asyncio.run(startup())
    ```
    """
    await FoundryReporterAgentExecutor.initialize_at_startup()