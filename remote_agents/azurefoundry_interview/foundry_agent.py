...
class FoundryTemplateAgent:
    ...
    This is a generic template agent that can be customized for any domain.
    It includes file search capabilities to ground responses in your uploaded documents.
    
    CUSTOMIZATION: Modify the _get_agent_instructions() method below to define your agent's behavior.
    ...
    def _get_agent_instructions(self) -> str:
        """
        Lead Interview Agent Instructions
        
        This agent is designed to conduct consultative interviews to qualify leads
        for an AI consulting company. It probes customers for detailed information
        about their needs, challenges, and context.
        """
        # Import configuration (done here to allow hot-reloading of config changes)
        try:
            from interview_config import (
                COMPANY_NAME, COMPANY_DESCRIPTION, AGENT_ROLE, 
                CONVERSATION_STYLE, PROBING_STRATEGIES, INTERVIEW_QUESTIONS,
                DOCUMENT_USAGE_INSTRUCTIONS, QUALIFICATION_CHECKLIST
            )
        except ImportError:
            # Fallback if config file is missing
            COMPANY_NAME = "Your AI Consulting Company"
            COMPANY_DESCRIPTION = "An AI consulting company"
            AGENT_ROLE = "Lead Qualification Specialist"
            CONVERSATION_STYLE = "Professional and consultative"
            PROBING_STRATEGIES = ""
            INTERVIEW_QUESTIONS = {}
        ...
        questions_summary = []
        for category, data in INTERVIEW_QUESTIONS.items():
            questions_summary.append(f"\n**{category.replace('_', ' ').title()} (Priority {data.get('priority', 0)})**")
            for q in data.get('questions', []):
                questions_summary.append(f"  - {q}")
        
        # Build qualification checklist
        checklist_items = []
        for key, description in QUALIFICATION_CHECKLIST.items():
            checklist_items.append(f"  - [ ] {description}")
