"""
AI Foundry Assessment & Estimation Specialist Agent implementation for multi-line loss analysis.
Adapted from the ADK agent pattern to work with Azure AI Foundry for damage inspections, cost estimation, and report preparation across auto, property, travel, health, and universal assessment scenarios.
...
class FoundryAssessmentAgent:
    """
    AI Foundry Assessment & Estimation Specialist Agent for multi-line damage appraisal and reporting.
    This class adapts the ADK agent pattern for Azure AI Foundry with focus on inspecting losses, applying cost estimation tables,
    and preparing structured assessment reports using the estimation reference documents for auto, property, travel, health, universal, and procedural scenarios.
    ...
    # Class-level shared resources for assessment document search (created once)
    ...
    async def _setup_file_search(self, files_directory: str = "documents") -> Optional[FileSearchTool]:
        ...
        async with FoundryAssessmentAgent._file_search_setup_lock:
            # If we already have a shared file search tool, return it
            if FoundryAssessmentAgent._shared_file_search_tool is not None:
                logger.info("Reusing existing shared file search tool")
                return FoundryAssessmentAgent._shared_file_search_tool
            ...
            try:
                ...
                for file_path in file_paths:
                    try:
                        ...
                        FoundryAssessmentAgent._shared_uploaded_files.append(file.id)
                ...
                FoundryAssessmentAgent._shared_vector_store = project_client.agents.vector_stores.create_and_poll(
                    file_ids=file_ids, 
                    name="shared_vectorstore"
                )
                logger.info(f"Created shared vector store: {FoundryAssessmentAgent._shared_vector_store.id}")
                
                # Create file search tool ONCE
                file_search = FileSearchTool(vector_store_ids=[FoundryAssessmentAgent._shared_vector_store.id])
                ...
                FoundryAssessmentAgent._shared_file_search_tool = file_search
    ...
    async def create_agent(self) -> Agent:
        ...
        if self.agent:
            logger.info("Assessment agent already exists, returning existing instance")
            return self.agent
    ...
    def _get_agent_instructions(self) -> str:
        """Get the agent instructions for multi-line assessment and estimation."""
        return f"""
You are an intelligent assessment and estimation specialist powered by Azure AI Foundry.

Your primary mission is to inspect losses, generate detailed cost estimates, assign severity, and prepare report-ready summaries using the estimation playbooks located in the `documents/` directory.

## Your Core Capabilities:

### 🚗 Auto Damage Assessment
- Use labor hour tables, parts pricing, and severity scales in `auto_estimation.md`.
- Capture collision vs. comprehensive scenarios, total-loss thresholds, and frame/paint considerations.

### 🏠 Property Damage Estimation
- Apply dwelling and contents cost tables from `property_estimation.md`.
- Account for depreciation, mitigation requirements, and structural vs. cosmetic impacts.

### ✈️ Travel & Trip Cost Modeling
- Estimate trip interruption, baggage replacement, and overseas medical costs with `travel_estimation.md` guidance.

### 🩺 Health Cost Benchmarking
- Reference procedure bundles, diagnostics, and inpatient stay ranges from `health_estimation.md`.

### 📑 Universal Workflow & QA
- Follow the workflow, severity matrix, regional multipliers, and depreciation rules in `universal_estimation.md`.
- Apply QA checklist and escalation steps from `procedures_faq.md`.

## Processing Instructions:

1. Capture inspection context, damage scope, and supporting figures.
2. Consult the appropriate estimation tables for labor, materials, and benchmarks.
3. Adjust for depreciation and regional multipliers; identify potential surge pricing.
4. Assign severity (minor/moderate/severe) and note escalation triggers.
5. Summarize recommended estimate (range or single value) with assumptions.
6. Provide documentation or reinspection requests when information is missing.

## Response Format:
```
📏 ASSESSMENT & ESTIMATION SUMMARY

**Loss Type**: [Auto/Property/Travel/Health]
**Scope Overview**: [Key damages or costs]
**Estimated Cost**: [Low–High or single value] (include labor vs. materials when possible)
**Severity & Triggers**: [Severity rating + rationale]
**Assumptions & Adjustments**: [Depreciation, multipliers, uncertainties]
**Recommended Actions**: [Inspections, mitigation, reinspection, QA notes]
**Reference Docs Used**: [e.g., auto_estimation.md §2, universal_estimation.md §3]
```

## Knowledge & Citation Guidelines
- Reference specific document names when citing cost tables or rules.
- If information is missing, state the gap and request follow-up.
- Highlight when surge pricing, senior sign-off, or escalations are required.

Current date and time: {datetime.datetime.now().isoformat()}

Remember: Deliver precise, defensible estimates that help adjusters make fast, confident decisions.
"""
...
async def create_foundry_assessment_agent() -> FoundryAssessmentAgent:
    """Factory function to create and initialize a Foundry Assessment & Estimation agent."""
    agent = FoundryAssessmentAgent()
...
async def demo_agent_interaction():
    """Demo function showing how to use the Foundry Assessment & Estimation agent for damage analysis."""
    ...
    try:
        ...
        # Example interaction
        message = "My basement flooded after a burst pipe and I need an estimate for repairs."
        print(f"\nUser: {message}")
        ...
        # await FoundryAssessmentAgent.cleanup_shared_resources()
