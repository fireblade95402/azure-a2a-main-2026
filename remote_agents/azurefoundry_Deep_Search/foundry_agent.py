...
class FoundryDeepSearchAgent:
    ...
    async def _setup_file_search(self, files_directory: str = "documents") -> Optional[FileSearchTool]:
        ...
        async with FoundryDeepSearchAgent._file_search_setup_lock:
            # If we already have a shared file search tool, return it
            if FoundryDeepSearchAgent._shared_file_search_tool is not None:
                logger.info("Reusing existing shared file search tool")
                return FoundryDeepSearchAgent._shared_file_search_tool
            ...
            try:
                ...
                for file_path in file_paths:
                    try:
                        ...
                        FoundryDeepSearchAgent._shared_uploaded_files.append(file.id)
                ...
                FoundryDeepSearchAgent._shared_vector_store = project_client.agents.vector_stores.create_and_poll(
                    file_ids=file_ids, 
                    name="shared_vectorstore"
                )
                logger.info(f"Created shared vector store: {FoundryDeepSearchAgent._shared_vector_store.id}")
                
                # Create file search tool ONCE
                file_search = FileSearchTool(vector_store_ids=[FoundryDeepSearchAgent._shared_vector_store.id])
                ...
                FoundryDeepSearchAgent._shared_file_search_tool = file_search
    ...
    async def create_agent(self) -> Agent:
        ...
        if self.agent:
            logger.info("Deep search agent already exists, returning existing instance")
            return self.agent
        ...
        with project_client:
            if tool_resources:
                self.agent = project_client.agents.create_agent(
                    model="gpt-4o",
                    name="foundry-deep-search-agent",
